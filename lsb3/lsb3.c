...
// To Get a Specific bit of Image bytes
		...
		do
    {
			...
			if(!feof(msg_file))
      {
        ...
        {  //Do this for every bit in every byte of the image
					...
					int lsb_bit = img_char & 1;// retrieving last bit
          int lsb2_bit = ((img_char>>1) & 1)+((img_char>>2) & 1)+lsb_bit;// retrieving second last bit
					msg_bit = get_bit(msg_char,i);

          lsb2_bit = (lsb2_bit%2==0)?0:1;
	...
	else
  {
		...
		for(int i = 0;i<message_length;i++)
    {
			...
			for( int j = 0;j<8;j++)
      {
				temp_ch = temp_ch<<1;
				img_char = fgetc(img_read);
				int lsb_bit = ((img_char & 1)+((img_char>>1) & 1)+((img_char>>2) & 1))%2;
        lsb_bit = (lsb_bit == 0)?0:1;
				if(lsb_bit == 1)
					temp_ch |= 1;
			}
